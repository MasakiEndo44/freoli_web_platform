...
## 10. Codex 互換運用（Claude Code と併用するためのルール）

このリポジトリは Claude Code での利用を前提に `.claude/` 配下へ settings / hooks / skills を配置している。
Codex も同じプロジェクト方針・ハーネス思想で動くが、Claude Code の実行設定を自動解釈しないため、
以下のルールで互換運用する。

### 基本方針

- Claude Code の実行設定（`.claude/settings.json`、`.claude/hooks/**`）は Claude 専用の runtime 設定として扱う。
- Codex は `.claude/settings.json` の permissions / hooks を自動実行しない。代わりに本ファイル、`CLAUDE.md`、`docs/harness/HARNESS_DECISIONS.md`、`.claude/skills/**/SKILL.md` を読んで同等の判断を行う。
- Codex 互換のためだけに `.claude/settings.json` や hook を変更しない。変更が必要な場合は「Claude ハーネス更新」として明示し、Layer 1 変更として Owner 承認を取る。
- Claude 用ファイルに Codex 専用の設定を混ぜない。Codex 向けの運用ルールは本 `AGENTS.md` に集約する。
- Claude と Codex のどちらで作業しても、Protected Areas / Gate 1 / Gate 2 / 検証コマンド / ブランチ規則は同じものを適用する。

### Codex の作業開始手順

Codex は実装・編集・調査を始める前に、タスクの重さに応じて以下を確認する。

1. `git status --short --branch` で作業ブランチと未コミット変更を確認する。
2. 本ファイルをプロジェクト共通ルールとして扱う。
3. `CLAUDE.md` を現在地点のスナップショットとして扱う。
4. Layer 1 / Layer 2 に触れる可能性がある場合は `docs/harness/HARNESS_DECISIONS.md` と `CONTENT_POLICY.md` も確認する。
5. 既存ドキュメントと実装が矛盾する場合は、より具体的な信頼源（要件定義書、`CONTENT_POLICY.md`、現行コード）を優先し、必要ならドキュメント更新も提案する。

### `.claude/skills` の扱い

Codex では Claude の slash command として skill は自動起動しない。
ただし、ユーザーの依頼が以下に該当する場合、対応する `SKILL.md` を全文読んでから、その手順を Codex の作業フローとして適用する。

| ユーザー意図 / トリガー | Codex が読む skill |
|---|---|
| 「議論して」「ブレストして」「専門家視点で」「みさきも交えて」 | `.claude/skills/expert-misaki-discussion/SKILL.md` |
| 「ゴールを詰めたい」「受け入れ基準」「実装前に整理」 | `.claude/skills/goal-grill/SKILL.md` |
| 「要件を詰めたい」「requirements-grill」「grill して」 | `.claude/skills/requirements-grill/SKILL.md` |
| 「要件定義」「仕様書」「Requirements」「PRD」 | `.claude/skills/requirements-doc-creator/SKILL.md` |
| `app/api/contact/**`、`lib/env.ts`、`.env*`、デプロイ・外部 API・破壊的操作に関わる変更 | `.claude/skills/security-sensitive-change-review/SKILL.md` |
| PR 本文・ドラフト要約・変更サマリー作成 | `.claude/skills/pr-draft-summary/SKILL.md` |
| skill の新規作成・改善・評価 | `.claude/skills/skill-creator/SKILL.md` または `.claude/skills/skill-improver/SKILL.md` |
| PostHog / Next.js App Router integration の追加 | `.claude/skills/integration-nextjs-app-router/SKILL.md` |

`SKILL.md` が references / assets / scripts を指示している場合、Codex は必要な関連ファイルだけを追加で読む。
Claude 専用コマンドや subagent 前提の手順がある場合は、Codex の available tools に置き換えて実行し、置き換え不能な部分はユーザーに明示する。

### Safety の置き換え

`.claude/hooks/prevent-destructive-command.js` は Codex では自動発火しないため、Codex は以下を手動ルールとして守る。

- `.env*` の実値を読まない・表示しない・コミットしない。
- `git push --force` / `git reset --hard` / 危険な `rm -rf` は、ユーザーが明示しても再確認する。
- Layer 1 ファイルの変更は、作業前に理由とリスクを説明する。
- Layer 2 コンテンツ（写真・ライブ情報・メンバー情報・ニュース・UI コピー）は、Content Owner 承認が必要な前提で扱う。
- 秘密値が疑われる文字列を見つけた場合は、値を再掲せず、rotation と履歴確認を促す。
